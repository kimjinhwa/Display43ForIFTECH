...
public:
    ...
    RtcDateTime GetDateTimeOneByOne()
    {
        uint8_t secReg  = getReg(0x80);  // DS1302_REG_TIMEDATE
        uint8_t minReg  = getReg(0x82);
        uint8_t hourReg = getReg(0x84);
        uint8_t dayReg  = getReg(0x86);
        uint8_t monReg  = getReg(0x88);
        uint8_t yearReg = getReg(0x8C);

        uint8_t second     = BcdToUint8(secReg & 0x7F);
        uint8_t minute     = BcdToUint8(minReg);
        uint8_t hour       = BcdToBin24Hour(hourReg);
        uint8_t dayOfMonth = BcdToUint8(dayReg);
        uint8_t month      = BcdToUint8(monReg);
        uint16_t year      = BcdToUint8(yearReg) + 2000;
