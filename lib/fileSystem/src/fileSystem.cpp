...
void LittleFileSystem::listDir(const char *path, char *match){
  ...
  if (!dir)
  {
    mySerialBT.printf( "Error opening directory\r\n");
    return;
  }
  ...
  uint64_t total = 0;
  int nfiles = 0;
  mySerialBT.printf( "T  Size      Date/Time         Name\r\n");
  mySerialBT.printf( "-----------------------------------\r\n");
  ...
  if (total)
  {
    mySerialBT.printf( "-----------------------------------\r\n");
    if (total < (1024 * 1024))
      mySerialBT.printf( "   %8d", (int)total);
    else if ((total / 1024) < (1024 * 1024))
      mySerialBT.printf( "   %6dKB", (int)(total / 1024));
    else
      mySerialBT.printf( "   %6dMB", (int)(total / (1024 * 1024)));
    mySerialBT.printf( " in %d file(s)\r\n", nfiles);
  }
  mySerialBT.printf( "-----------------------------------\r\n");
  ...
  mySerialBT.printf( "SPIFFS: free %d KB of %d KB\r\n", (tot - used) / 1024, tot / 1024);
  mySerialBT.printf( "-----------------------------------\r\n");
}
...
int LittleFileSystem::rm(String fileName)
{
  ...
  while ((entry = readdir(dir)) != NULL)
  {
        ...
        strcmp(entry->d_name + strlen(entry->d_name) - ext_len, unLinkfilename.c_str()) == 0)
    {
      ...
      mySerialBT.printf( "deleted file %s\r\n", filePath.c_str());
    }
  }
}
...
void LittleFileSystem::df()
{
  ...
  mySerialBT.printf("| Type | Sub |  Offset  |   Size   |       Label      |\r\n");
  mySerialBT.printf("| ---- | --- | -------- | -------- | ---------------- |\r\n");
  ...
  if (pi != NULL)
  {
    do
    {
      ...
      mySerialBT.printf("|  %02x  | %02x  | 0x%06X | 0x%06X | %-16s |\r\n",
